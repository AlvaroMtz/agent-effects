...
- [x] 1. `EffectJournal.findRequest(runId, effectId)` in core, implemented by both backends.
...
- [x] 2. `@agent-effects/replay` — `createReplayRuntime({ journal })`, hit path.
...
- [x] 4. Offline guarantees and the replay demo.
...
All five work units complete on `feat/replay-minimal`.

| Check | Result |
|-------|--------|
| `pnpm -r test` | exit 0 — core 40 reported (20 distinct), journal-memory 15, journal-jsonl 27, replay 13 |
| `pnpm -r typecheck` | exit 0 |
| `pnpm -r build` | exit 0 |

Distinct tests: 75, up from 60.

### Exit criteria

| Criterion | Evidence |
|-----------|----------|
| Replay is 100% offline | `createReplayRuntime` takes only a journal; there is no executor parameter, so an external call is unrepresentable |
| No external calls | The demo replays `fixtures/journal/basic-run.jsonl` with `fetch` stubbed to throw; it is never called |
| Tests verify zero IO | A test reads the package's own sources and asserts none imports `node:*` or names a network API |
| Mismatch detection | `ReplayMismatchError` names the field and both values; `ReplayMissError` distinguishes an unrecorded occurrence from an unresolved one |

### Deviations

1. **`EffectJournal.findRequest` added to core** (work unit 1). Not a new
   decision: ADR-0004 §1 already states that replay finds the recorded
   request/resolution pair, and ADR-0012 §2 put the whole effect in the request
   entry so it could be verified. The interface was missing the half that makes
   mismatch detection possible. No ADR was written, because none was needed.
2. **Matching ignores `metadata`.** Adapter data is outside the portable
   contract (ADR-0008), so replaying with a different provider request id is
   not a divergence. Recorded in the package README so it is a stated rule
   rather than an accident of the comparison.
3. **`canonicalize` is exported.** The fingerprint is part of how a mismatch is
   reported, so a consumer diagnosing one needs to be able to reproduce it. It
   is diagnostic only and never an identity (ADR-0002 §2).
